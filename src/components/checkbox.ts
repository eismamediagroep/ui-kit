...
// No `shadow` here: Chrome renders box-shadow on native <input> form controls
// as a rectangle around the bounding box, ignoring border-radius, which can
// produce a visible square halo around the control.
const CONTROL_CLASS =
  'h-4 w-4 shrink-0 rounded-sm border border-neutral-300 ' +
  'text-neutral-900 ' +
...
export class EismaCheckbox extends EismaElement {
  protected mount(): void {
    ...
    CONSUMED_LABEL.forEach((a) => this.removeAttribute(a));
